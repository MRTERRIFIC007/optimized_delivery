...
class DeliveryChatbot:
    def __init__(self, predictor, perplexity_api_key=None):
        ...
        self.chat_history = []
        self.session = requests.Session()  # Create a session for better connection reuse
    ...
    def process_query(self, query, current_context=None):
        ...
        try:
            ...
            try:
                # Call Perplexity API with increased timeout and retries
                ...
                logging.error("Timeout while calling Perplexity API")
                return self._generate_mock_response(query, system_context)
            except requests.exceptions.RequestException as e:
                logging.error(f"Request error while calling Perplexity API: {str(e)}")
                return self._generate_mock_response(query, system_context)
...
This route prioritizes morning delivery for Ananya who has the lowest failure rate in mornings, followed by afternoon deliveries for others based on their availability patterns."""
        ...
        if "today" in query and "deliveries" in query:
            ...
            try:
                ...
                delivery_list = []
                for i, order in enumerate(pending_orders[:5], 1):  # Limit to first 5 orders
                    ...
                    delivery_list.append(f"{i}. **#{order_id} - {name} (Pkg: {package_size}) - {area}, {address}.**")
                ...
                try:
                    ...
                    if weather_data:
                        weather_conditions = self.predictor._get_weather_summary(weather_data)
                except Exception as e:
                ...
                try:
                    ...
                    if traffic_data:
                        traffic_conditions = self.predictor._get_traffic_summary(traffic_data)
                except Exception as e:
                ...
                try:
                    ...
                    if festival_data:
                        festival_conditions = self.predictor._get_festival_summary(festival_data)
                except Exception as e:
